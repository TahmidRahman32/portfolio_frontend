...
import { clearOAuthParams, getCurrentUser, getOAuthCallbackStatus, initiateOAuthLogin, verifyOAuthAuthentication } from "@/components/services/auth/oauth-handler";
...
export default function LoginPage() {
     ...
     const [state, formAction, isPending] = useActionState<ActionState, FormData>(submitActionLogin, { errors: {}, success: false });

     // ✅ FIXED: Handle OAuth callback
     useEffect(() => {
        const handleOAuthCallback = async () => {
           const callbackStatus = getOAuthCallbackStatus();

           // Only process if we have an error
           if (!callbackStatus?.error && callbackStatus?.isCallback !== true) {
              return;
           }
           ...
           const toastId = toast.loading("Verifying your login...");
           ...
           try {
              ...
              if (isAuthenticated) {
                 ...
                 toast.success("Login Successful!", {
                    id: toastId,
                    description: `Welcome back, ${user?.first_name || user?.name || "user"}!`,
                    duration: 2000,
                 });
                 ...
                 toast.error("Authentication Failed", {
                    id: toastId,
                    description: "Cookies may not be set. Try again or use email login.",
                    duration: 5000,
                 });
                 clearOAuthParams();
                 setOauthLoading(null);
     ...
     const handleGoogleLogin = async () => {
        try {
           ...
           // Initiate OAuth - this will redirect
           ...
           setTimeout(() => {
              ...
              toast.error("Google Login", {
                 description: "Failed to initiate. Please try again.",
                 duration: 4000,
              });
           }, 3000);
        } catch (error) {
     ...
     const isLoading = isPending || !!oauthLoading || isVerifyingOAuth;
         ...
         <motion.div initial={{ opacity: 0, y: 28 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, ease }} className="relative z-10 w-full max-w-md">
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.65, ease, delay: 0.18 }} className="rounded-2xl border border-white/[0.08] bg-white/[0.03] backdrop-blur-sm p-7 md:p-8">
               ...
               <div className="space-y-2.5 mb-6">
                  <SocialBtn onClick={handleGoogleLogin} disabled={isLoading} isLoading={oauthLoading === "google"}>
               ...
               <form action={formAction} className="space-y-4" noValidate>
                  ...
                  <div>
                     ...
                     <div className="relative">
                        ...
                        <input name="email" type="email" value={email} onChange={(e) => setEmail(e.target.value)} autoComplete="email" placeholder="you@example.com" className={fieldCls(!!state.errors?.email)} disabled={isLoading} />
                     </div>
                     <AnimatePresence>
                        {state.errors?.email && (
                  ...
                  <div className="flex items-center justify-between pt-1">
                     <label className="flex items-center gap-2 cursor-pointer select-none group">
                        <div className={`w-4 h-4 rounded border flex items-center justify-center transition-all duration-200 ${rememberMe ? "border-white/40 bg-white/10" : "border-white/[0.12] bg-white/[0.03]"}`}>
                           <input type="checkbox" name="rememberMe" checked={rememberMe} onChange={(e) => setRememberMe(e.target.checked)} className="sr-only" disabled={isLoading} />
...
//          // Only process if we detect OAuth callback params
//          if (!callbackStatus?.isCallback) {
//             return;
//          }
