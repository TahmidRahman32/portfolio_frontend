...
//          <input ref={fieldRef} name={name} value={value} onChange={onChange} onBlur={onBlur} type={show ? "text" : "password"} placeholder="••••••••" autoComplete="current-password" className={`${fieldCls(invalid)} pr-10`} />
...
//          setAuthError("");
//          const tid = toast.loading("Signing you in…", { description: "Verifying your credentials." });
...
//                toast.error("Login failed", { id: tid, description: "Check your email and password.", duration: 4000 });
//             } else {
//                toast.success("Welcome back!", { id: tid, description: "Redirecting to your dashboard.", duration: 2500 });
//                router.push("/dashboard");
//             }
//          } catch {
//             setAuthError("An unexpected error occurred. Please try again.");
//             toast.error("Something went wrong", { id: tid, duration: 4000 });
//          }
//       });
//    };
...
//                               <div className={`w-4 h-4 rounded border flex items-center justify-center transition-all duration-200 ${field.value ? "border-white/40 bg-white/10" : "border-white/[0.12] bg-white/[0.03]"}`}>
//                                  <input type="checkbox" checked={!!field.value} onChange={(e) => field.onChange(e.target.checked)} className="sr-only" />
//                                  {field.value && <div className="w-2 h-2 rounded-sm bg-white/70" />}
//                               </div>
